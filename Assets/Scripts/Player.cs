...
public class Player : MonoBehaviour
{
    ...
    private bool isHavePopcorn = false;
    ...
    {
        if (collision.gameObject != null && collision.gameObject.CompareTag("Popcorn"))
        {
            if (!isHavePopcorn)
            {
                isHavePopcorn = true;

                GameObject vfx = Instantiate(vfxCollected, transform.position, Quaternion.identity) as GameObject;
                Destroy(vfx, 1f);

                collision.gameObject.transform.position = holder.position;
                collision.gameObject.transform.SetParent(holder);
            }
            else
            {
                startMovement = false;
        ...
        if (collision.gameObject != null && collision.gameObject.CompareTag("Target"))
        {
            if (!isHavePopcorn)
            {
                startMovement = false;
            ...
            else
            {
                ...
                GameManager.Instance.popcorns.Remove(holder.GetChild(0).gameObject);
